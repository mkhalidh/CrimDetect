...
const adminController = {
    ...
    async addCriminal(req, res, next) {
        try {
            ...
            // Check if person with CNIC already exists
            if (cnic) {
                const existingPerson = await Person.findByCnic(cnic);
                if (existingPerson) {
                    return response.error(res, 'Person with this CNIC already exists', 409);
                }
            }
            ...
            if (user_id) {
                ...
                if (person) {
                    ...
                    await Person.update(person.id, {
                        ...
                        cnic,
                        image_url: image_url || person.image_url, // Keep old image if no new one
                        face_descriptor: descriptor || person.face_descriptor,
                        status: 'CRIMINAL'
                    });
                ...
                // Create standalone person record
                person = await Person.create({
                    name,
                    age: parseInt(age) || null,
                    cnic,
