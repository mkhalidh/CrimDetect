...
import { ShieldAlert, Mail, Lock, User, Eye, EyeOff, Loader2 } from 'lucide-react';
...
export default function AdminRegister() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await authService.registerAdmin({ name, email, password });

            if (response.success) {
                login(response.data.user);
                showToast({
                    title: 'Admin Account Created!',
                    description: `Welcome, Admin ${response.data.user.name}!`,
                    variant: 'success',
                });
                navigate('/admin');
            }
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-950 via-purple-950 to-slate-900 p-4">
            ...
            <div className="relative z-10 w-full max-w-md">
                ...
                <div className="text-center mb-8">
                    <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-purple-500 to-pink-600 shadow-2xl shadow-purple-500/30 mb-4">
                        <ShieldAlert className="h-8 w-8 text-white" />
                    </div>
                    <h1 className="text-3xl font-bold text-white">CrimDetect Admin</h1>
                    <p className="text-purple-200 mt-2">Administrative Access</p>
                </div>

                <Card className="bg-white/10 backdrop-blur-xl border-white/20">
                    <CardHeader className="space-y-1">
                        <CardTitle className="text-2xl text-white text-center">Create Admin Account</CardTitle>
